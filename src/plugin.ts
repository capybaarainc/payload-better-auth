...
export const betterAuthPlugin = (options?: BetterAuthPluginOptions): Plugin => {
  return (config: Config): Config => {
    ...
    if (options?.auth) {
      const usersCollection = collections.find(c => c.slug === 'users' || c.slug === config.admin?.user);
      if (usersCollection) {
            ...
            ...((typeof usersCollection.auth === 'object' && usersCollection.auth?.strategies) || []),
            {
              ...
              authenticate: async ({ headers, payload }) => {
                try {
                  ...
                  if (!res || !res.session || !res.user) return { user: null };
                  const fullUser = await payload.findByID({ collection: usersCollection.slug as any, id: res.user.id });
                  if (!fullUser) return { user: null };
                  return { user: { ...fullUser, id: res.user.id as any, collection: usersCollection.slug, _strategy: 'better-auth' } as any };
                } catch (error) {
                  return { user: null };
                }
              }
